...
#define kNERtcMaxChannelNameLength  128     /**< 频道ID最大长度 */
#define kNERtcMaxTokenLength        256     /**< token最大长度 */
...
typedef void* source_id_t;  /**< 用于标识屏幕捕捉的源，代表某个窗口或屏幕。源为窗口时, Windows上该类型会转换为HWND, MAC上转换为整形。源为屏幕时会转换为整形*/
...
struct NERtcScreenCaptureParameters
{
    ...
    int bitrate;                        /**< 共享视频的码率，单位为 kbps；默认值为 0，表示 SDK 根据当前共享屏幕的分辨率计算出一个合理的值。*/
    bool capture_mouse_cursor;          /**< 是否采集鼠标用于屏幕共享。*/
    bool window_focus;                  /**< 调用 startScreenCaptureByWindowId 方法共享窗口时，是否将该窗口前置。*/
    source_id_t* excluded_window_list;  /**< 待屏蔽窗口的 ID 列表。 */
...
/** 视频流类型 */
typedef enum {
    kNERTCVideoStreamMain = 0, /**< 主流 */
    kNERTCVideoStreamSub = 1,  /**< 辅流 */
    kNERTCVideoStreamCount,    /**< 预留参数，无需关注。 */
...
struct NERtcTextWatermarkConfig {
	...
	*/
    ...
    char font_path[kNERtcMaxURILength]; /**< 字体路径，设置为空时，表示使用程序默认字体。*/
	int font_size;			/**< 字体大小。默认值为 10，相当于 144 dpi 设备上的 10 x 15 磅。*/
	int font_color;			/**< 字体颜色。ARGB 格式。 */
	int offset_x;			/**< 水印左上角与视频画布左上角的水平距离。单位为像素（pixel）。*/
	int offset_y;			/**< 水印左上角与视频画布左上角的垂直距离。单位为像素（pixel）。*/
	int wm_color;			/**< 水印框内背景颜色。ARGB格式，支持透明度设置。*/
	int wm_width;			/**< 水印框的宽度。单位为像素（pixel），默认值为 0，表示没有水印框。*/
	int wm_height;			/**< 水印框的高度。单位为像素（pixel），默认值为 0，表示没有水印框。*/
};
...
struct NERtcTimestampWatermarkConfig {
    char font_path[kNERtcMaxURILength]; /**< 字体路径。若未设置，使用程序默认字体。*/
	int font_size;			/**< 字体大小。默认值为 10，相当于 144 dpi 设备上的 10 x 15 磅。*/
	int font_color;			/**< 字体颜色。ARGB 格式。 */
	int offset_x;			/**< 水印左上角与视频画布左上角的水平距离。单位为像素（pixel）。 */
	int offset_y;			/**< 水印左上角与视频画布左上角的垂直距离。单位为像素（pixel）。 */
...
struct NERtcChannelMediaRelayConfiguration {
    NERtcChannelMediaRelayInfo* src_infos;  /**< 当前房间的信息，即原始房间信息 */
    NERtcChannelMediaRelayInfo* dest_infos; /**< 转发媒体流的目标房间配置信息 */
    int dest_count;                         /**< 目标频道数量，默认值为 0 */
};
...
struct NERtcImageWatermarkConfig {
	char image_paths[10][kNERtcMaxURILength];/**< 水印图片路径。空时无效。*/
	int offset_x;	/**< 水印图片左上角与视频画布左上角的水平距离。单位为像素（pixel），默认值为 0。 */
	int offset_y;    /**< 水印图片左上角与视频画布左上角的垂直距离。单位为像素（pixel），默认值为 0。 */
    int image_width; /**< 水印图片的宽度。单位为像素（pixel），默认值为 0 表示按原始图宽。*/
    int image_height;/**< 水印图片的高度。单位为像素（pixel），默认值为 0 表示按原始图高。*/
	int fps;		/**< 播放帧率。默认 0 帧/秒，即不自动切换图片，图片单帧静态显示。注意：Windows端帧率不超过 20 fps。*/
	bool loop;		/**< 是否设置循环。默认循环，设置为 false 后水印数组播放完毕后消失。*/
};
...
struct NERtcCanvasWatermarkConfig {
    NERtcImageWatermarkConfig* image_watermarks;        /**< 图片水印数组指针。 */
    int image_count_;                                   /**< 图片水印个数，最多支持 4 个。 */
    NERtcTextWatermarkConfig* text_watermarks;          /**< 文字水印数组指针。 */
    int text_count_;                                    /**< 文字水印个数，最多支持 10 个。 */
	NERtcTimestampWatermarkConfig* timestamp_watermark;   /**< 时间戳水印指针，仅一个。 */
};
...
struct NERtcServerAddresses {
    ...
    bool use_ipv6;		                         /**< 是否使用IPv6（默认false) */
};
...
/** bool value，其他用户打开视频时，自动订阅。默认值 false。通话前设置有效
@note 
- 用户可以修改订阅状态。但是自动订阅后，如果对端重新发布，则会恢复自动订阅的状态。
- 该接口仅windows下有效。
*/
...
#define kNERtcKeyChannel1V1ModeEnabled      "channel_1v1_mode_enabled"  /**< bool value. 1v1模式开关，默认关闭。通话前设置有效 */
} // namespace nertc
