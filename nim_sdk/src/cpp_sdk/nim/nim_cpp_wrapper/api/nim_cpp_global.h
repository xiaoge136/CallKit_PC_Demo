...
public:
    ...
    /** @fn void DetectProxy(NIMProxyType type, const std::string& host, int port, const std::string& user, const std::string& password, const
     * DetectProxyCallback& callback) 测试代理
     * @param[in] type 代理类型，见NIMProxyType定义
     * @param[in] host 代理地址
     * @param[in] port 代理端口
     * @param[in] user 代理用户名
     ...
     * @return void 无返回值
     */
    static void DetectProxy(NIMProxyType type,
    ...
    /** @fn void GetSDKCachedFileInfoAsync(const std::string &login_id, const std::string &file_type, int64_t end_timestamp, const std::string
     * &json_extension, const GetCachedFileInfoCallback &cb); 获取sdk缓存文件信息
     ...
     * @return void 无返回值
     */
    static void GetSDKCachedFileInfoAsync(const std::string& login_id,
                                          const std::string& file_type,
                                          int64_t end_timestamp,
                                          const std::string& json_extension,
                                          const GetCachedFileInfoCallback& cb);

    /** @fn void DeleteSDKCachedFileAsync(const std::string &login_id, const std::string &file_type, int64_t end_timestamp, const std::string
     &json_extension, const DeleteCachedFileCallback &cb)
     * 删除sdk缓存文件
     * @param[in] login_id 查询的账号ID
     * @param[in] file_type 文件类型，常量定义见nim_global_def.h 查询SDK文件缓存信息文件类型file_type
     ...
     * @return void 无返回值
     */
    static void DeleteSDKCachedFileAsync(const std::string& login_id,
                                         const std::string& file_type,
                                         int64_t end_timestamp,
                                         const std::string& json_extension,
