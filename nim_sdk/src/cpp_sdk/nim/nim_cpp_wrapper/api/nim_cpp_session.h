...
public:
    ...
    typedef std::function<void(nim::NIMResCode, const std::list<SessionData>& session_data_list, int unread_count)>
        SetMultiUnreadCountZeroCallback;                                                       /**< 多条会话未读消息数清零回调模板 */
    ...
    typedef std::function<void(const std::string& session_id, nim::NIMSessionType)>
        CancelStickTopSessionNotifyCallback;                                                           /**< 取消置顶会话通知回调模板 */
    ...
    typedef std::function<void(nim::NIMResCode, const std::string& session_id, nim::NIMSessionType)>
        CancelStickTopSessionCallback;                                                                         /**< 取消置顶会话回调模板 */
    ...
    typedef std::function<void(nim::NIMResCode, const SessionRoamMsgHasMoreTagInfo& info)>
        QueryHasmoreRoammsgCallback; /**< 查询会话是漫游消息未拉取信息回调模板 */
    typedef std::function<void(nim::NIMResCode, const std::list<SessionRoamMsgHasMoreTagInfo>& info_list)>
        QueryAllHasmoreRoammsgCallback;                                        /**< 查询所有漫游消息未拉取完全的会话回调模板 */
    ...
    typedef std::function<void(nim::NIMResCode, nim::NIMSessionType, const std::string& session_id)>
        DeleteSessionRoammsgCallback; /**< 删除会话是漫游消息未拉取信息回调模板 */
     ...
     * @return void 无返回值
     */
    static void QueryAllRecentSessionAsync(NIMMessageType last_msg_excluded_type,
                                           const QuerySessionListCallabck& cb,
                                           const std::string& json_extension = "");
