...
static void CallbackDetectProxy(bool connect, NIMProxyDetectStep step, const char* json_extension, const void* user_data) {
    CallbackProxy::DoSafeCallback<Global::DetectProxyCallback>(
        user_data, [=](const Global::DetectProxyCallback& cb) { CallbackProxy::Invoke(cb, connect, step, PCharToString(json_extension)); }, true);
}
...
static void CallbackDeleteCachedFile(enum NIMResCode code, const void* user_data) {
    CallbackProxy::DoSafeCallback<Global::DeleteCachedFileCallback>(
        user_data, [=](const Global::DeleteCachedFileCallback& cb) { CallbackProxy::Invoke(cb, code); });
}
...
static void CallbackSDKFeedback(enum NIMResCode code, const void* user_data) {
    CallbackProxy::DoSafeCallback<Global::SDKFeedbackCallback>(user_data,
                                                               [=](const Global::SDKFeedbackCallback& cb) { CallbackProxy::Invoke(cb, code); });
}
    ...
    (
        ...
        [](int rescode, const void* callback) {
            CallbackProxy::DoSafeCallback<SDKFeedbackCallback>(
                callback, [=](const SDKFeedbackCallback& cb) { CallbackProxy::Invoke(cb, (NIMResCode)rescode); }, true);
        },
        callback);
}
}  // namespace nim
