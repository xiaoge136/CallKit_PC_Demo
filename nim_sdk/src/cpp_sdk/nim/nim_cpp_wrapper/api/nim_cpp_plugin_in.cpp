...
static void CallbackRequestChatRoomEnter(int error_code, const char* result, const char* json_extension, const void* user_data) {
    CallbackProxy::DoSafeCallback<PluginIn::ChatRoomRequestEnterCallback>(
        user_data, [=](const PluginIn::ChatRoomRequestEnterCallback& cb) { CallbackProxy::Invoke(cb, error_code, PCharToString(result)); });
}
