...
typedef void (*nim_session_update_session_hasmore_roammsg)(const char* json_msg,
...
void Session::QueryAllRecentSessionAsync(NIMMessageType last_msg_excluded_type,
...
void Session::QueryAllRecentSessionAsyncEx(const std::list<NIMMessageType>& last_msg_excluded_type,
    ...
    (json_extension.c_str(), &CallbackQuerySession, nim_cpp_wrapper_util::Json::FastWriter().write(json_excluded_type_list).c_str(), cb_pointer);
}
    ...
    (
        ...
        [](int rescode, const char* to_type, const char* id, const void* user_data) {
            CallbackProxy::DoSafeCallback<DeleteSessionRoammsgCallback>(
                user_data,
                [=](const DeleteSessionRoammsgCallback& cb) {
                    CallbackProxy::Invoke(cb, (nim::NIMResCode)rescode, (nim::NIMSessionType)std::atoi(to_type), PCharToString(id));
                },
                true);
        },
        cb_pointer);
...
bool Session::SetMultiUnreadCountZeroAsync(bool super_team,
    ...
    (
        super_team, nim_cpp_wrapper_util::Json::FastWriter().write(json_list).c_str(),
        ...
        [](int rescode, const char* session_id, enum NIMSessionType to_type, const void* user_data) {
            CallbackProxy::DoSafeCallback<CancelStickTopSessionCallback>(
                ...
                [=](const CancelStickTopSessionCallback& cb) {
                    CallbackProxy::Invoke(cb, (nim::NIMResCode)rescode, PCharToString(session_id), to_type);
                },
                true);
        },
        cb_pointer);
