...
typedef void (*nim_talkex_collect_query_collects)(const char* query_collect_list_param,
    ...
    (
        ...
        [](int code, int count, const void* user_data) {
            CallbackProxy::DoSafeCallback<RemoveCollectsCallback>(
                user_data, [=](const RemoveCollectsCallback& cb) { CallbackProxy::Invoke(cb, code, count); }, true);
        },
        cb_pointer);
}
