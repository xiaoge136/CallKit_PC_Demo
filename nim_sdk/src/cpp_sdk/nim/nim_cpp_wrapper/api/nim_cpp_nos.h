...
public:
    ...
    typedef std::function<void(NIMResCode res_code, const std::string& file_path, const std::string& call_id, const std::string& res_id)>
        DownloadMediaCallback;                                                                    /**< 下载回调模板 */
    ...
    typedef std::function<void(const std::string& url, std::string& token)> CustomTokenCallback;                 /**< 自定义token回调模板 */
    ...
    /** @fn static void RegCustomCustomTokenCb(const CustomTokenCallback& cb);
     * (全局回调)注册自定义token获取回调
     * @param[in] cb 回调函数
     * @return void 无返回值
     */
    ...
    /** @fn static bool FetchMediaEx(const IMMessage& msg,
     ...
     *                其他:参见HTTP协议或CURL错误码
     */
    static bool FetchMediaEx(const IMMessage& msg,
                             const std::string& json_extension,
                             const DownloadMediaCallback& callback_result,
                             const ProgressCallback& callback_progress,
                             const SpeedCallback& callback_speed = SpeedCallback(),
    ...
    /** @fn static bool UploadResource(const std::string& local_file,
     ...
     *                其他:参见HTTP协议或CURL错误码
     */
    static bool UploadResource(const std::string& local_file,
                               const UploadMediaCallback& callback_result,
                               const ProgressCallback& callback_progress = ProgressCallback());

    /** @fn static bool UploadResource2(const std::string& local_file,
     ...
     *                其他:参见HTTP协议或CURL错误码
     */
    static bool UploadResource2(const std::string& local_file,
                                const std::string& tag,
                                const UploadMediaCallback& callback_result,
                                const ProgressCallback& callback_progress = ProgressCallback());

    /** @fn static bool UploadResourceEx(const std::string& local_file,
                                         const std::string& json_extension,
     ...
     * 上传资源
     * @param[in] local_file 本地文件的完整路径，路径名必须是utf-8
     * @param[in] json_extension json扩展参数，拼接字段见 nim_nos_def.h "NOS扩展上传\下载接口参数json_extension Json key for nim_nos_upload_ex &&
     *nim_nos_download_ex"
     ...
     *                其他:参见HTTP协议或CURL错误码
     */
    static bool UploadResourceEx(const std::string& local_file,
                                 const std::string& json_extension,
                                 const UploadMediaExCallback& callback_result,
                                 const ProgressExCallback& callback_progress = ProgressExCallback(),
                                 const SpeedCallback& callback_speed = SpeedCallback(),
    ...
    /** @fn static bool UploadResourceEx2(const std::string& local_file,
                                          const std::string& tag,
                                          const std::string& json_extension,
                                          const UploadMediaExCallback& callback_result,
                                          const ProgressExCallback& callback_progress = ProgressExCallback(),
     ...
     *                其他:参见HTTP协议或CURL错误码
     */
    static bool UploadResourceEx2(const std::string& local_file,
                                  const std::string& tag,
                                  const std::string& json_extension,
                                  const UploadMediaExCallback& callback_result,
                                  const ProgressExCallback& callback_progress = ProgressExCallback(),
    ...
    /** @fn static bool DownloadResource(const std::string& nos_url,
                                         const DownloadMediaCallback& callback_result,
                                         const ProgressCallback& callback_progress = ProgressCallback())
     * 下载资源
     ...
     *                其他:参见HTTP协议或CURL错误码
     */
    static bool DownloadResource(const std::string& nos_url,
                                 const DownloadMediaCallback& callback_result,
                                 const ProgressCallback& callback_progress = ProgressCallback());

    /** @fn static bool DownloadResourceEx(const std::string& nos_url,
                                           const std::string& json_extension,
     ...
     *                其他:参见HTTP协议或CURL错误码
     */
    static bool DownloadResourceEx(const std::string& nos_url,
                                   const std::string& json_extension,
                                   const DownloadMediaExCallback& callback_result,
                                   const ProgressExCallback& callback_progress = ProgressExCallback(),
                                   const SpeedCallback& callback_speed = SpeedCallback(),
