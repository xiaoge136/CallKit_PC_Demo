...
typedef void (
    *nim_sysmsg_query_msg_async)(int limit_count, int64_t last_time, const char* json_extension, nim_sysmsg_query_cb_func cb, const void* user_data);
...
static void CallbackQuerySysmsg(int count, const char* result, const char* json_extension, const void* callback) {
    CallbackProxy::DoSafeCallback<SystemMsg::QueryMsgCallback>(
        callback,
        [=](const SystemMsg::QueryMsgCallback& cb) {
...
static void CallbackNotifySingleSysmsg(int res_code, int64_t msg_id, int unread_count, const char* json_extension, const void* callback) {
    CallbackProxy::DoSafeCallback<SystemMsg::SetStatusCallback>(
        callback, [=](const SystemMsg::SetStatusCallback& cb) { CallbackProxy::Invoke(cb, (NIMResCode)res_code, msg_id, unread_count); }, true);
}

static void CallbackNotifySysmsgRes(int res_code, int unread_count, const char* json_extension, const void* callback) {
    CallbackProxy::DoSafeCallback<SystemMsg::NotifySysmsgResCallback>(
        callback, [=](const SystemMsg::NotifySysmsgResCallback& cb) { CallbackProxy::Invoke(cb, (NIMResCode)res_code, unread_count); }, true);
}
...
void SystemMsg::SetStatusByTypeAsync(NIMSysMsgType type,
                                     NIMSysMsgStatus status,
