...
public:
    ...
    public:
        ...
        using AddQuickCommentNotifyCallback =
            std::function<void(const std::string&, NIMSessionType, const std::string&, const QuickCommentInfo&)>; /**<添加快捷回复通知回调*/
        using RemoveQuickCommentNotifyCallback = std::function<void(const std::string& session,
                                                                    NIMSessionType type,
