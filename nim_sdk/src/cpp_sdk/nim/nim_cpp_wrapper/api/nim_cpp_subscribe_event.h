...
public:
    ...
    typedef std::function<void(NIMResCode res_code, const std::list<nim::EventData>& event_list)>
        BatchPushEventCallback; /**< 批量接收订阅事件回调模板 */
    ...
    typedef std::function<void(NIMResCode res_code, int event_type, const std::list<std::string>& faild_list)>
        SubscribeEventCallback; /**< 订阅事件回调模板 */
    typedef std::function<void(NIMResCode res_code, int event_type, const std::list<std::string>& faild_list)>
        UnSubscribeEventCallback; /**< 按账号取消指定事件的订阅关系回调模板 */
    typedef std::function<void(NIMResCode res_code, int event_type)> BatchUnSubscribeEventCallback; /**< 取消指定事件的全部订阅关系回调模板 */
    typedef std::function<void(NIMResCode res_code, int event_type, const std::list<nim::EventSubscribeData>& subscribe_list)>
        QuerySubscribeEventCallback; /**< 按帐号查询指定事件的订阅关系回调模板 */
    typedef std::function<void(NIMResCode res_code, int event_type, const std::list<nim::EventSubscribeData>& subscribe_list)>
        BatchQuerySubscribeEventCallback; /**< 查询指定事件的全部订阅关系回调模板 */
...
public:
    ...
    /** @fn static bool Subscribe(int event_type,
                                  int64_t ttl,
     ...
     *                500:未知错误
     */
    static bool Subscribe(int event_type,
                          int64_t ttl,
                          NIMEventSubscribeSyncEventType sync_type,
                          const std::list<std::string>& accid_list,
                          const SubscribeEventCallback& cb,
    ...
    /** @fn static bool UnSubscribe(int event_type,
                                    const std::list<std::string>& accid_list,
     ...
     *                500:未知错误
     */
    static bool UnSubscribe(int event_type,
                            const std::list<std::string>& accid_list,
                            const UnSubscribeEventCallback& cb,
                            const std::string& json_extension = "");
    ...
    /** @fn static bool QuerySubscribe(int event_type,
                                       const std::list<std::string>& accid_list,
     ...
     *                500:未知错误
     */
    static bool QuerySubscribe(int event_type,
                               const std::list<std::string>& accid_list,
                               const QuerySubscribeEventCallback& cb,
                               const std::string& json_extension = "");
};
