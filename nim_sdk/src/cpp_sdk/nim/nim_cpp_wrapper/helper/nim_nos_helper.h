...
struct NIM_SDK_CPPWRAPPER_DLL_API InitNosConfigParam {
     ...
     * @return void
     */
    void RemoveTag(const std::string& tag) { tag_list_.erase(tag); }
    /** @fn std::map<std::string, uint64_t> GetTagList() const
     * @return std::map<std::string, uint64_t> 场景标签信息列表
     */
    std::map<std::string, int32_t> GetTagList() const { return tag_list_; }
    /** @fn void SetGetExtension(const std::string& json_extension)
     * @param[in] json_extension 扩展数据
     * @return void
     */
    void SetGetExtension(const std::string& json_extension) { json_extension_ = json_extension; }
    /** @fn std::string GetExtension() const
     * @return string  扩展数据
     */
    std::string GetExtension() const { return json_extension_; }
 ...
 * @return bool 解析成功 或失败
 */
NIM_SDK_CPPWRAPPER_DLL_API bool ParseDownloadResult(const std::string& file_path,
