...
struct NIM_SDK_CPPWRAPPER_DLL_API MessageSetting {
    ...
    int anti_apam_using_yidun_; /**< int,  (可选) 单条消息是否使用易盾反垃圾 0:(在开通易盾的情况下)不过易盾反垃圾而是通用反垃圾
                                   其他都是按照原来的规则 */
    ...
    std::string anti_spam_ext;        /**< (可选)String, 易盾反垃圾扩展字段，限制 json，长度限制 1024 */
    std::string anti_spam_res;        /**< String, 易盾反垃圾返回的结果字段 */
...
struct NIM_SDK_CPPWRAPPER_DLL_API IMFile {
    ...
    std::string msg_attachment_tag_; /**< string, (可选)发送含有附件的消息时使用的场景标签(可参见nos删除策略) ,Audio Image Video File
                                        或者可以被SDK解析到本地文件路径的自定义消息 */
    /** 构造函数 */
    IMFile()
        : size_(0)
