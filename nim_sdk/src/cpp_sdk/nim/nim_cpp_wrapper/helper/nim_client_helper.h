...
struct NIM_SDK_CPPWRAPPER_DLL_API SDKConfig {
    ...
    bool animated_image_thumbnail_enabled_; /**< bool 开启对动图缩略图的支持
    ...
    std::string default_nos_upload_host_; /**< string 默认nos 上传服务器主机地址，仅 kNIMUseHttps设置为true 时有效，用作 https 上传时的域名校验及 http
                                             header host 字段填充 */
    ...
    std::string nos_accelerate_host_; /**< string 需要被加速主机名【8.1.0版本以及后不建议使用，推荐nos_accelerate_host_list_】 */
    std::vector<std::string> nos_accelerate_host_list_; /**< string array，需要被加速主机名列表 */
    ...
    bool priority_use_cdn_host_;                                 /**< bool 是否优先使用cdn域名 */
    bool disable_app_nap_;                                       /**< string 是否禁用 macOS 下的 App Nap 功能，默认为 true  */
    bool cache_session_data_when_delete_;                        /**< bool 删除会话时是否在数据库缓存会话原数据，默认为 false */
    NIMSDKType sdk_type;                                         /**< 客户端sdk类型 */
    std::string sdk_human_version;                               /**< sdk可读版本号 */

    /***********融合存储相关************/
    std::string mock_refer_;
    std::string mock_ua_;
    int fcs_auth_type_; /**< 融合云存储认证方式 1=refer鉴权,2=基于时间的token鉴权，3=基于url的token鉴权，4=custom鉴权 */
    bool custom_enable_fcs_; /**< 是否开启融合云存储 */
