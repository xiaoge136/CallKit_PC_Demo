...
Value::UInt Value::asUInt() const {
    switch (type()) {
        ...
        case stringValue:
            try {
                return std::stoul(value_.string_);
            } catch (...) {
                return 0;
            }
        default:
...
Value::UInt64 Value::asUInt64() const {
    switch (type()) {
        ...
        case stringValue:
            try {
                return std::stoull(value_.string_);
            } catch (...) {
                return 0;
            }
        default:
...
Value& Value::operator[](ArrayIndex index) {
    JSON_ASSERT_MESSAGE(type() == nullValue || type() == arrayValue,
                        "in nim_cpp_wrapper_util::Json::Value::operator[](ArrayIndex): requires arrayValue");
    if (type() == nullValue)
        *this = Value(arrayValue);
    CZString key(index);
...
const Value& Value::operator[](ArrayIndex index) const {
    JSON_ASSERT_MESSAGE(type() == nullValue || type() == arrayValue,
                        "in nim_cpp_wrapper_util::Json::Value::operator[](ArrayIndex)const: requires arrayValue");
    if (type() == nullValue)
...
Value& Value::resolveReference(const char* key) {
    JSON_ASSERT_MESSAGE(type() == nullValue || type() == objectValue,
                        "in nim_cpp_wrapper_util::Json::Value::resolveReference(): requires objectValue");
    if (type() == nullValue)
...
Value& Value::resolveReference(char const* key, char const* end) {
    JSON_ASSERT_MESSAGE(type() == nullValue || type() == objectValue,
                        "in nim_cpp_wrapper_util::Json::Value::resolveReference(key, end): requires objectValue");
    if (type() == nullValue)
        *this = Value(objectValue);
...
Value::Members Value::getMemberNames() const {
    JSON_ASSERT_MESSAGE(type() == nullValue || type() == objectValue,
                        "in nim_cpp_wrapper_util::Json::Value::getMemberNames(), value must be objectValue");
    if (type() == nullValue)
