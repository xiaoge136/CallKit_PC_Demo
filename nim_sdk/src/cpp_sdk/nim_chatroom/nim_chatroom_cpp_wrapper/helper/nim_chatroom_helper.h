...
struct NIM_CHATROOM_SDK_CPPWRAPPER_DLL_API ChatRoomIndependentEnterInfo {
    ...
    std::list<std::string> login_tags_; /**< 登录标签，可以设置多个 */
    std::string notify_tags_;           /**< 登录登出通知的目标标签 */
    NIMChatRoomLocation location_;      /**< 位置坐标描述 */
    bool enable_location_;              /**< 是否启用空间消息能力，true 为启用，false 为关闭，SDK 不解析 location 相关信息 */
...
struct NIM_CHATROOM_SDK_CPPWRAPPER_DLL_API ChatRoomNotification {
    ...
    int64_t temp_mute_duration_; /**<当通知为临时禁言相关时有该值，禁言时代表本次禁言的时长(秒)，解禁时代表本次禁言剩余时长(秒);
    ...
    /** Added IM 8.4.0 */
    std::string target_tag_; /**< 当通知为聊天室按标签禁言/解禁事件才有，代表当前被禁言的 tag */

    /** Added IM 8.7.0 */
    std::string msg_id_;   /**< 当通知为聊天室撤回消息事件时才有，代表当前被撤回的消息 ID */
    uint64_t msg_timetag_; /**< 当通知为聊天室撤回消息事件时才有，代表当前被撤回消息的时间戳 */
...
public:
    bool transient_; /**< true 表示当提交这个新元素的用户从聊天室掉线或退出的时候，需要删除这个元素；默认 false 表示不删除 */
    std::string element_accid_; /**< 队列元素所属账号，默认不传表示队列元素属于当前操作人，管理员可以指定队列元素归属于其他合法账号 */
...
struct NIM_CHATROOM_SDK_CPPWRAPPER_DLL_API ChatRoomMessageSetting {
    ...
    int anti_spam_using_yidun_; /**< int,  (可选) 单条消息是否使用易盾反垃圾 0:(在开通易盾的情况下)不过易盾反垃圾而是通用反垃圾
                                   其他都是按照原来的规则 */
    ...
    std::string anti_spam_ext_;       /**< (可选)String, 易盾反垃圾扩展字段，限制json，长度限制1024 */
    NIMChatRoomLocation location_;    /**< (可选)发送消息时附加的位置信息 */
    /** 构造函数 */
...
public:
    ...
    // --> v7.8
    // v8.4.0 <--
    std::string notify_tags_; /**< 对端发送消息或通知时携带的 notify tags 信息 */
    // --> v8.4.0
    std::string anti_spam_res_; /**<  String, 易盾反垃圾返回的结果字段*/
...
struct NIM_CHATROOM_SDK_CPPWRAPPER_DLL_API ChatRoomMemberInfo {
    ...
    std::string tags_;       /**<聊天室成员登录时指定的 tag 信息 */
    ...
    std::string notify_tags_;      /**<聊天室成员登录时指定的 notify_tags 信息 */
