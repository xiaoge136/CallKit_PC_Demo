...
public:
    ...
    typedef std::function<
        ...
        EnterCallback; /**< 登录回调, 如果错误码为kResRoomLocalNeedRequestAgain，聊天室重连机制结束，则需要向IM服务器重新请求进入该聊天室权限 */
    ...
    typedef std::function<void(int64_t room_id, int error_code, const std::list<ChatRoomMemberInfo>& infos)>
        GetMembersCallback;                                                                              /**< 在线查询成员列表回调 */
    typedef std::function<void(int64_t room_id, int error_code, uint64_t)> GetMembersCountByTagCallback; /**< 在线查询成员列表回调 */
    typedef std::function<void(int64_t room_id, int error_code, const std::list<ChatRoomMessage>& msgs)>
        GetMsgHistoryCallback; /**< 在线查询消息历史回调 */
    typedef std::function<void(int64_t room_id, int error_code, const ChatRoomMemberInfo& info)>
        SetMemberAttributeCallback; /**< 设置成员身份标识回调 */
    ...
    typedef KickMemberCallback UpdateLocationCallback; /**< 更新聊天室中用户的位置信息回调 */
    ...
    typedef std::function<void(int64_t room_id, int error_code, const std::list<std::string>& not_in_queue)>
        QueueBatchUpdateCallback; /**< 队列批量更新回调 */
