...
/** @typedef void (*nim_user_special_relationship_change_cb_func)(enum NIMUserSpecialRelationshipChangeType type,
                                                                  const char *result_json,
                                                                  const char *json_extension,
                                                                  const void *user_data)
 * 用户多端同步类型（黑名单、静音名单）的监听回调函数定义
 * @param[out] type        NIMUserSpecialRelationshipChangeType，用户多端同步类型
 * @param[out] result_json        同步内容，kNIMUserSpecialRelationshipChangeTypeMarkBlack: {"accid" : "abc", "black" : bool} ,
 * kNIMUserSpecialRelationshipChangeTypeMarkMute: {"accid" : "abc", "mute" : bool}, kNIMUserSpecialRelationshipChangeTypeSyncMuteAndBlackList: {a, b,
 * c ...}(a,b,c为json value array - 用户特殊关系（黑名单、静音名单） Json Keys)
...
/** @typedef void (*nim_user_sync_muteandblacklist_cb_func)(int res_code,
                                                            const char *black_list_json,
                                                            const char *json_extension,
                                                            const void *user_data)
 * 获取黑名单、静音列表
 * @param[out] res_code        结果代码，一切正常200
 * @param[out] black_list_json        黑名单和静音列表Json字符串（{a , b , ...}(a,b,c为json value array - 用户特殊关系（黑名单、静音名单） Json
 * Keys)）
 * @param[out] json_extension    json扩展数据（备用）
 * @param[out] user_data        APP的自定义用户数据，SDK只负责传回给回调函数，不做任何处理！
 * @return void 无返回值
 ...
 * 用户名片的监听回调函数定义
 * @param[out] result_json    json object array 例 [{ "accid" : "litianyi01", "create_timetag" : 1430101821372, "gender" : 0, "name" : "oleg01",
 * "update_timetag" : 1430101821372 }, ...]
 * @param[out] json_extension    json扩展数据（备用）
 * @param[out] user_data        APP的自定义用户数据，SDK只负责传回给回调函数，不做任何处理！
 * @return void 无返回值
 */
