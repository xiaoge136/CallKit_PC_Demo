...
enum NIMAudioDataCbType {
    ...
    kNIMAudioDataCbTypeHookAndMic =
        2, /**< 定时返回伴音和麦克风、声卡的混音数据，允许重采样（json中带kNIMDeviceSampleRate和kNIMVolumeWork），返回单声道数据 */
};
 ...
 */
/// string 设备名称
static const char* kNIMDeviceName = "name";
/// string 设备路径
static const char* kNIMDevicePath = "path";
/// int32 采样频率
static const char* kNIMDeviceSampleRate = "sample_rate";
/// int32 采样位深
static const char* kNIMDeviceSampleBit = "sample_bit";
/// 用户id int64
static const char* kNIMDeviceDataUid = "uid";
/// 用户账号 string
static const char* kNIMDeviceDataAccount = "account";
/// int32 画面宽
static const char* kNIMDeviceWidth = "width";
/// int32 画面高
static const char* kNIMDeviceHeight = "height";
/// int32 视频数据类型，NIMVideoSubType
static const char* kNIMVideoSubType = "subtype";
/// string 标识ID
static const char* kNIMDeviceId = "id";
/// int32 大于0标识数据将使用音量参数进行换算(暂时只针对伴音数据)
static const char* kNIMVolumeWork = "volume_work";
/// int 大于0时标记为耳机设备
static const char* kNIMDeviceHeadphone = "headphone";
...
/** @typedef void (*nim_vchat_device_status_cb_func)(enum NIMDeviceType type,
                                                     uint32_t status,
 ...
 * 当kNIMDeviceStatusReset状态时需要关注此参数，kNIMDeviceStatusReset时有可能选用了非用户选定的设备，这里返回的是重新启动的设备地址
...
/** @typedef void(*nim_vchat_audio_data_cb_func_ex)(uint64_t time,
                                                    const char *data,
                                                    uint32_t size,
                                                    int channels,
                                                    int rate,
...
/** @typedef void (*nim_vchat_audio_data_cb_func)(uint64_t time, const char *data, unsigned int size,
...
/** @typedef void (*nim_vchat_video_data_cb_func)(uint64_t time, const char *data, unsigned int size, unsigned int width,
                                                  unsigned int height, const char *json_extension, const void *user_data)
 * NIM Device 视频数据监听接口
 * @param[out] time 时间毫秒级
