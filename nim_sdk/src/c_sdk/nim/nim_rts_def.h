...
/** @typedef void (*nim_rts_start_cb_func)(int code, const char *session_id, int channel_type, const char *uid,
 ...
 */
typedef void (
    *nim_rts_start_cb_func)(int code, const char* session_id, int channel_type, const char* uid, const char* json_extension, const void* user_data);

/** @typedef void (*nim_rts_start_notify_cb_func)(const char *session_id, int channel_type, const char *uid,
 *                                                const char *json_extension, const void *user_data)
 * NIM RTS 收到对方创建通道通知
 * @param[out] session_id 会话id
 * @param[out] channel_type 通道类型 如要tcp+音视频，则channel_type=kNIMRtsChannelTypeTcp|kNIMRtsChannelTypeVchat
 * @param[out] uid 对方帐号
 * @param[out] json_extension 扩展字段，若有透传数据见kNIMRtsCreateCustomInfo，通道id见kNIMRtsChannelId，如{"custom_info": "","channel_id":
 * 123213213123 }
 * @param[out] user_data APP的自定义用户数据，SDK只负责传回给回调函数，不做任何处理！
 * @return void 无返回值
 */
typedef void (
    *nim_rts_start_notify_cb_func)(const char* session_id, int channel_type, const char* uid, const char* json_extension, const void* user_data);
...
/** @typedef void (*nim_rts_ack_res_cb_func)(int code, const char *session_id, int channel_type, bool accept,
 *                                           const char *json_extension, const void *user_data)
 * NIM RTS 回复收到邀请的结果
 * @param[out] code 调用结果
 * @param[out] session_id 会话id
 ...
 */
typedef void (
    *nim_rts_ack_res_cb_func)(int code, const char* session_id, int channel_type, bool accept, const char* json_extension, const void* user_data);

/** @typedef void (*nim_rts_ack_notify_cb_func)(const char *session_id, int channel_type, bool accept, const char *uid,
 *                                              const char *json_extension, const void *user_data)
 * NIM RTS 收到对方回复邀请的通知
 * @param[out] session_id 会话id
 * @param[out] channel_type 通道类型
 ...
 * @return void 无返回值
 */
typedef void (*nim_rts_ack_notify_cb_func)(const char* session_id,
                                           int channel_type,
                                           bool accept,
                                           const char* uid,
...
/** @typedef void (*nim_rts_sync_ack_notify_cb_func)(const char *session_id, int channel_type, bool accept,
 *                                                   const char *json_extension, const void *user_data)
 ...
 */
typedef void (
    *nim_rts_sync_ack_notify_cb_func)(const char* session_id, int channel_type, bool accept, const char* json_extension, const void* user_data);

// 状态回调相关
/** @typedef void (*nim_rts_connect_notify_cb_func)(const char *session_id, int channel_type, int code,
 *                                                  const char *json_extension, const void *user_data)
 * NIM RTS 通道连接状态通知
 * @param[out] session_id 会话id
 * @param[out] channel_type 通道类型
...
/** @typedef void (*nim_rts_member_change_cb_func)(const char *session_id, int channel_type, int type, const char *uid,
 *                                                 const char *json_extension, const void *user_data)
 * NIM RTS 通道连接成员变化通知
 * @param[out] session_id 会话id
 * @param[out] channel_type 通道类型
 ...
 * @return void 无返回值
 */
typedef void (*nim_rts_member_change_cb_func)(const char* session_id,
                                              int channel_type,
                                              int type,
                                              const char* uid,
                                              const char* json_extension,
...
// 控制接口
/** @typedef void (*nim_rts_control_res_cb_func)(int code, const char *session_id, const char *info,
 *                                               const char *json_extension, const void *user_data)
 * NIM RTS 控制接口调用结果
 * @param[out] code 调用结果
 * @param[out] session_id 会话id
...
/** @typedef void (*nim_rts_control_notify_cb_func)(const char *session_id, const char *info, const char *uid,
 *                                                  const char *json_extension, const void *user_data)
 * NIM RTS 控制消息通知回调
 * @param[out] session_id 会话id
 * @param[out] info 透传数据
 ...
 */
typedef void (
    *nim_rts_control_notify_cb_func)(const char* session_id, const char* info, const char* uid, const char* json_extension, const void* user_data);
...
/** @typedef void (*nim_rts_rec_data_cb_func)(const char *session_id, int channel_type, const char *uid, const char *data,
 *                                            unsigned int size, const char *json_extension, const void *user_data)
 * NIM RTS 数据监听回调
 ...
 * @return void 无返回值
 */
typedef void (*nim_rts_rec_data_cb_func)(const char* session_id,
                                         int channel_type,
                                         const char* uid,
                                         const char* data,
                                         unsigned int size,
